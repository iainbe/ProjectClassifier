...
- Every new test/pilot row carries `codebook_version=1.1-candidate` and `review_batch` (e.g. R2-DIAGNOSTIC, R2-TRANSFER, R3-PILOT). Historical labels remain in review history, not silently relabelled.
...
Use the `claim_type` field. For pure CONTEXT, `effect_family=NOT_APPLICABLE`. For METHOD_OUTPUT, DIRECT is appropriate only for the contracted output itself; method reuse may instead describe internal capability and have no intervention effect family. For DESIGN, classify the intended effect, set timing FORECAST and assess occurrence separately from the existence of the design document. Avoid making one row assert both design existence and realised downstream benefit.
...
Attribution: NOT_ASSESSED / DESCRIPTIVE / CONTRIBUTION / CAUSAL_ESTIMATE / NOT_APPLICABLE. DESCRIPTIVE reports an association, not a causal effect. CONTRIBUTION requires explicit alternatives/chronology/mechanism evidence. CAUSAL_ESTIMATE requires an identified estimand, credible design, assumptions, data adequacy and specialist review where warranted. A distinctive approach does not qualify by itself.
...
Add `value_basis`: OBSERVED_AMOUNT / DESCRIPTIVE_ESTIMATE / SCENARIO_ESTIMATE / NONE / UNKNOWN / NOT_APPLICABLE; and `value_review_status`: NOT_REVIEWED / REVIEWED / DISPUTED / NOT_APPLICABLE. Costs/prices are not estimated benefits: `amount_role` on measurements is COST / BENEFIT / ECONOMIC_BASELINE / OTHER / NOT_APPLICABLE / UNKNOWN.

...
| Field | Value | Source |
|---|---|---|
| `project_id` | P03-MITIH | 01_projects.csv |
| `project_name` | MITIH Createch Ecosystem — MediaCity Immersive Technologies Innovation Hub | 01_projects.csv + Iain confirmation |
...
| Field | Value | Source |
|---|---|---|
| `methods_used` | Labour market analysis (job postings 2020-2024, skill-demand trends); social network analysis (bridging organisation identification); direct sector-leader engagement; case documentation (3-way prototype); gated investment sequencing; cost-per-initiative estimation | C-R3-009-013, C-G2-412-426 |
| `method_innovation` | Gated investment sequence — demand validation gate before Phase 1 ecosystem strengthening before Phase 2 expansion, with three explicit market validation questions (graduate supply vs demand, AI impact, etc.); SNA applied to identify microbusiness bridging function rather than just cluster size | C-R3-013, C-G2-421/425/426 |
| `method_evolution` | Three-month research sprint early 2025; single report + policy playbook | changelog |
| `breakthrough_points` | SNA reframes microbusinesses as bridging infrastructure (not just small firms); gated sequence converts "fund the ecosystem" into a testable investment logic with explicit validation gates | C-G2-422/426 |
| `spillover_types_identified` | KNOWLEDGE (skill-demand shift to collaborative capability); NETWORK (microbusiness bridging, 3-way prototype); PRODUCT (integrated immersive production); OPTION (gated investment sequence) | claims register |
| `spillover_evidence_strength` | KNOWLEDGE: METHOD_OUTPUT (postings analysis); NETWORK: METHOD_OUTPUT + EFFECT (documented prototype case); PRODUCT: EFFECT (Realtime VP department); OPTION: DESIGN (proposed, unexercised) | Derived |
| `option_analysis` | Gated sequence: immediate demand validation (3-4 months) → Phase 1 ecosystem strengthening → Phase 2 — option held by client/funders | C-R3-013, C-G2-426 |
| `causal_analysis` | DESCRIPTIVE + documented case — one microbusiness's 3-way prototype is testimony-documented contribution, not counterfactual | C-R3-012, C-G2-423 |
| `measurement_approach` | Quantitative (job postings, skill trends, UKRI award counts, equipment costs) + SNA + qualitative testimony | claims register |

### B2. Repeatability and transferability

| Field | Value | Source |
|---|---|---|
| `repeatability` | HIGH — labour market analysis + SNA + gated investment sequence is repeatable for any sector ecosystem; requires postings data and network data | Derived |
| `transfer_conditions` | Job postings data source; SNA tooling; sector-leader access; funder relationships for gated sequence credibility | Derived |
| `data_dependencies` | Job postings data (2020-2024), UKRI award records, organisational network data, sector-leader testimony | claims register |
| `access_requirements` | Postings data platform; sector network access; MITIH/consortium relationships | Derived |
| `effort_estimate` | ~3 months research (early 2025) | report |
| `reuse_examples` | Gated investment sequence and SNA bridging analysis inform option-design methodology across later mapping projects | Derived |

### B3. Tender relevance

| Field | Value | Source |
|---|---|---|
| `tender_relevance_tags` | createch-ecosystem, immersive-technologies, SNA-analysis, gated-investment-sequence, labour-market-analysis, mediaciity, salford, PRIME | Derived |
| `buyer_types` | Innovation hub, local authority, Innovate UK programme | Derived |
| `precedent_strength` | STRONG — PRIME, accepted, methodologically distinctive (SNA + gated sequence), not-confidential status aids citation | Derived |
| `precedent_caveats` | Initial report rejected for recommendations (Oct 24 reconciliation accepted) — tender use must be honest about revision history; job postings are demand proxy not employment; gated sequence unexercised; 2025 project — limited post-delivery outcome evidence; single geography (MediaCity) | Derived |
| `positioning_notes` | Demonstrates the option-design capability in its most explicit form — gated investment sequencing is the strategic-option-value method made concrete | Derived |

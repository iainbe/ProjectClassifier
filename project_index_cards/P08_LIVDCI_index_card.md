...
**Card status:** REVIEWED — populated from full read of canonical final report (SRC-R2-09B); consistency confirmed by Iain 2026-09-12 (PRIME role, client acceptance, rating split).
...
| `commission_date` | ~2024-01 (proposal Jan 5, 2024) | file system metadata |
| `completion_date` | 2024-07-08 (final report accepted by client) | canonical version record + Iain confirmation |
| `contract_value` | NOT_ESTABLISHED | 06_measurements.csv |
| `contracting_role` | PRIME — Iain confirmed 2026-09-12 | VAL-R3-015 |
...
| `lifecycle_status` | COMPLETED — final report accepted by client; Iain confirmed 2026-09-09 | 01_projects.csv |
...
| `key_deliverables` | (1) Final report: LiverpoolCityRegion_DigitalCreative_final (Jul 8, 2024, DOCX+PDF, 844 paras, 27 tables, 85 pages); (2) Proposal: LCR DCI Cluster mapping (Jan 5, 2024, DOCX, 274 paras); (3) Changes document (Jul 8, 2024, DOCX) | canonical version records |
...
| `evidence_strength` | MEDIUM — substantial quantitative mapping (27 tables, 85 pages); PRIME contractor; multiple data sources; innovative methodology; but headline GVA figures are modelled estimates not observed outcomes; Iain confirmed split rating 2026-09-12 | Derived |
...
| `method_evolution` | 3 phases: proposal (Jan 2024, rev=307) → outline and drafting (Jun 2024, 4 versions in 3 days) → final report (Jul 2024, table count jumped 6→27). Final report went through outline → draft → final in ~1 month | changelog |
...
| `effort_estimate` | ~6 months (Jan–Jul 2024) | Notes |
